...
const categories = ['For You', 'Worship', 'Sermons', 'Podcasts', 'Kids', 'Live', 'Downloads'];
        ...
        <View style={{ paddingHorizontal: theme.spacing.lg }}>
          <FadeIn>
            ...
            <CustomText style={{ color: theme.colors.text.secondary, marginTop: 4 }}>
              Search across audio, video, and live sessions.
            </CustomText>
          </FadeIn>
          ...
          <FadeIn delay={160}>
            <ScrollView
              ...
              contentContainerStyle={{ paddingVertical: theme.spacing.md, gap: theme.spacing.sm }}
            >
              {categories.map((cat) => (
          ...
          <FadeIn delay={280}>
            <MediaRail
              title="Top creators"
              actionLabel="See all"
              onAction={() => console.log('all creators')}
              data={topCreators}
              renderItem={(item) => (
                <PosterCard
                  key={item.id}
                  ...
                  title={item.title}
                  subtitle={item.subtitle}
                  onPress={() => console.log('creator', item.id)}
                  size="sm"
                />
              )}
            />

  ...
  <AuthLayout>
    ...
    <form v-if="!mfaRequired" class="space-y-5" @submit.prevent="onLogin">
      <div class="text-center mb-6">
        ...
        <p class="text-sm text-ink-muted mt-1">Sign in to your admin account</p>
      </div>

      <div v-if="auth.error" class="p-3.5 rounded-xl bg-danger/10 border border-danger/20 text-sm text-danger">
        {{ auth.error }}
      </div>
      ...
      <AppButton type="submit" :loading="auth.isLoading" full-width size="lg" class="mt-2">
        Sign in
      </AppButton>

      <div v-if="googleLoginUrl" class="relative flex items-center gap-3 my-2">
    ...
    <form v-else class="space-y-5" @submit.prevent="onMfa">
      <div class="text-center mb-6">
        <div class="w-12 h-12 rounded-2xl bg-primary/15 flex items-center justify-center mx-auto mb-3">
        ...
        <h2 class="text-xl font-bold text-ink">Two-factor verification</h2>
        <p class="text-sm text-ink-muted mt-1">Enter the 6-digit code from your authenticator app</p>
      </div>
      ...
      <AppButton variant="ghost" full-width size="sm" @click="mfaRequired = false; mfaToken = ''">
        ← Back to sign in
      </AppButton>
    </form>
  </AuthLayout>

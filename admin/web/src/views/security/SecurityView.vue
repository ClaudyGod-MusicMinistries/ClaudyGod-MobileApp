<template>
  <AppPage eyebrow="Account protection" title="Secure your administrator account" description="Multi-factor authentication is required before privileged workspace access is granted.">
    <!-- Recovery codes are deliberately the highest-priority state. Session
         refresh marks MFA active immediately after verification, but the codes
         are shown only once and must remain visible until the administrator
         explicitly confirms safe storage. -->
    <AppCard v-if="backupCodes.length" class="max-w-2xl p-6">
      <div class="space-y-5">
    ...
    <AppCard v-else-if="auth.user?.mfaEnabled" class="max-w-2xl p-6">

...
// ─── Curated daily plan ──────────────────────────────────────────────────────
// 52 verses — one per week, cycled by ISO week number.
// Each entry has: ref (bible-api.com compatible), theme (card label), reflection.

const DAILY_PLAN: { ref: string; theme: string; title: string; reflection: string }[] = [
  ...
  { ref: 'joshua 1:9',         theme: 'Courage',            title: 'Be Strong & Courageous',   reflection: 'God commanded courage because He knew you'd need it. Take that step you've been putting off.' },
  ...
  { ref: 'romans 5:8',         theme: 'Grace',              title: 'While We Were Sinners',    reflection: 'God didn't wait for you to be worthy to love you. That is grace. Receive it fully today.' },
  ...
  { ref: 'james 1:5',          theme: 'Wisdom',             title: 'Ask God for Wisdom',       reflection: 'You don't have to figure everything out alone. Ask God for wisdom — He gives it generously, without reproach.' },
  ...
  { ref: 'luke 1:37',          theme: 'Nothing Impossible', title: 'Nothing Impossible with God', reflection: 'That situation you've written off as impossible — bring it back to God. Nothing is beyond Him.' },
  ...
  { ref: 'romans 12:2',        theme: 'Transformation',     title: 'Be Transformed',           reflection: 'Renewal begins in the mind. Don't be shaped by the world — let God's word reshape how you think.' },
  { ref: 'john 10:10',         theme: 'Abundant Life',      title: 'Life to the Full',         reflection: 'Jesus didn't come to give you a mediocre life. He came that you might have life in abundance.' },
  { ref: 'matthew 5:14-16',    theme: 'Light of the World', title: 'You Are the Light',        reflection: 'You are not just a light — you are THE light of the world. Don't hide it; let it shine before everyone.' },
  ...
  { ref: 'nehemiah 8:10',      theme: 'Joy as Strength',    title: 'The Joy of the Lord',      reflection: 'Your joy doesn't depend on your circumstances. It comes from the Lord and it is your strength.' },
  ...
  { ref: 'isaiah 55:8-9',      theme: 'God\'s Ways Higher', title: 'My Ways Are Higher',       reflection: 'When you don't understand what God is doing — trust that He sees things from a perspective you can't yet reach.' },
...
export function getTodaysPlan(date = new Date()): (typeof DAILY_PLAN)[number] {
  const index = getDayOfYear(date) % DAILY_PLAN.length;
  return DAILY_PLAN[index]!;
}
...
export async function fetchBibleDailyVerse(): Promise<WordOfDayItem> {
  ...
  try {
    ...
    // Extract clean reference from the formatted ref (e.g. "John 3:16")
    passage = plan.ref
      .replace(/\+/g, ' ')
      .split(' ')
    ...
    // Network failure — still show the theme with empty verse text
  }
...
/** Clear the daily cache (e.g., for testing). */

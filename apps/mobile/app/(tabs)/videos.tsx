...
export default function VideosScreen() {
  ...
  return (
    <PremiumPage title="Videos" subtitle="Watch new releases, live replays, worship sessions, and featured ministry videos." eyebrow="Watch" refreshing={loading} onRefresh={refresh} rightAction={<AppButton title="Search" variant="secondary" size="sm" onPress={() => router.push(APP_ROUTES.tabs.search)} leftIcon={<MaterialIcons name="search" size={16} color={theme.colors.text} />} />}>
      ...
      <ContentRail title="Up next" items={queue} compact onPressItem={(item) => void openItem(item, 'videos_queue')} />
      <ContentRail title="Latest videos" items={feed.videos} onPressItem={(item) => void openItem(item, 'videos_latest')} />
      <ContentRail title="Live & replays" items={feed.live} compact onPressItem={(item) => void openItem(item, 'videos_live')} />
      ...
      {!loading && !queue.length ? <EmptyState title="No videos available yet" message="New videos and live replays will appear here when they are ready." actionLabel="Refresh" onAction={refresh} icon="smart-display" /> : null}
    </PremiumPage>
  );
}
